                         ...
                         float* __restrict__ x_k1)
{
    ...
    float t = 0 ;
    //const int sh_size = 5120 ;  
    const int n_shIter = ((n / blockDim.x)==0)? 1 : n/blockDim.x ; 
...
void GPU_Shared( int n , int iter , float* input , float* sol , float* x_k , float* x_k1 ){
    ...
    for ( int it = 0 ; it < iter ; it++){
        Shared_kernel<<< numBlock , blocksize , n*sizeof(float) >>> ( n , numBlock , d_input , d_sol , d_xk , d_xk1 ) ;
        printf( "iter = %d \n" , it ) ;
